 ...
 *                                                                        *
 *                         PortSniffer v 2.0                              *
 *                     Main class for port sniffer                        *
 *                                                                        *
 *                       Copyright (c) 2020 LYL                           *
 *                            @author LYL                                 *
 *                            @version 2.0                                *
 **************************************************************************/
public class sniffGUI extends Group implements Observer {
    ...
    private ShowPane showPane = new ShowPane();
    private ProgressBar progressBar = new ProgressBar(WINDOW_WIDTH - 100, 5, true);
    private MenuButton menuButton = new MenuButton();
    private SettingGroup settingGroup = new SettingGroup();
    private boolean showSetting = false;
    private Stage primaryStage;

    public sniffGUI(double width, double height, Stage primaryStage) {
    ...
    public sniffGUI(double width, double height) {
        this.WINDOW_HEIGHT = height;
        this.WINDOW_WIDTH = width;
        sniff.reg(this);
        init();
    ...
    private void init(){
        showPane.setSize(this.WINDOW_WIDTH,this.WINDOW_HEIGHT-150);
        settingGroup.resize(this.WINDOW_WIDTH,this.WINDOW_HEIGHT);
        ...
        settingGroup.setLayoutY(WINDOW_HEIGHT);
        //-- component initial end --//

        this.getChildren().addAll(startButton, clearButton, address_In, showPane, progressBar, settingGroup, menuButton);
    ...
    public void resize(Number width, Number height, double ZOOMFACTOR) {
        ...
        settingGroup.root.setLayoutY(height.doubleValue() * 0.3);
        settingGroup.root.setLayoutX((width.doubleValue() - settingGroup.root.getWidth()) / 2);
        settingGroup.resize(width, height);
        if (!showSetting)
            settingGroup.setLayoutY(height.doubleValue());
        ...
        showPane.setSize(width.doubleValue() - 50, height.doubleValue() - 200);
        showPane.setLocation(25, 150);

        progressBar.setSize(width.doubleValue() - 100, 5);
        progressBar.setLocation(width.doubleValue() / 2 - progressBar.getComponentWidth() / 2, height.doubleValue() - 30);
    }
    ...
    private boolean checkIllegal(String input){
        ...
        return (input.length()>=1 && input.contains(".") || input.equals("localhost")) && input.charAt(input.length()-1)!='.';
    }
}
